"""Phase 7 — RULER NIAH 4k quality gate at TurboQuant K3-V2 KV.

Mirrors scripts/phase6_run_ruler_4k_int4.py but the patched backend uses
PersistentTurboKVCache (--kv-bits 3 equivalent). Compares dense SDPA vs
patched-Turbo. Gate: patched_turbo_hits / dense_hits >= 0.85 for every task.
"""
...
def run_patched_turbo(model_name, ctx_len, n_samples, seed, max_new,
    ...
    print(f"\n=== patched TurboQuant K3-V3 (retention={retention}, all-retrieval) ===")
    model, tok = load_awq_model(model_name)
    cfg = model.config
...
def main():
    ...
    for t in TASKS:
        ...
        if not passed:
            all_pass = False
        tasks_out[k] = {
            "dense_hits": d, "patched_turbo_hits": q,
            "total": dense[k]["total"], "ratio": ratio, "pass": passed,
            "dense_wall_s": dense[k]["wall_s"],
            "patched_turbo_wall_s": patched[k]["wall_s"],
        }
    ...
    result = {
        ...
        "seed": args.seed, "head_pattern": "all-retrieval", "kv_bits": 3,
        "tasks": tasks_out,
        "gate": "≥85% vs dense at retention=0.25 + all-retrieval head_pattern + TurboQuant K3-V2",
        "all_pass": all_pass,
    }
    if args.out:
        out_path = Path(args.out)
    else:
        out_path = Path(__file__).resolve().parents[1] / "benchmarks" / "phase7_ruler_4k_turbo.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(result, indent=2))
    print(f"\nWrote {out_path}\nAll-pass: {all_pass}")
    for t, info in tasks_out.items():
        print(f"  {t}: {info['patched_turbo_hits']}/{info['dense_hits']} "
